...
This internal variant is intended for corporate intranet environments with SSL interception.
All Python `requests` calls in bundled scripts use `verify=False`, and the Node fetch script
disables TLS certificate validation globally.
...
```bash
python $SKILL_DIR/scripts/fetch_html.py --in [keyword]/.cache/annotated_results.json --html-dir [keyword]/.cache/html --out [keyword]/.cache/prefetched.json
node $SKILL_DIR/scripts/fetch_content.mjs --in [keyword]/.cache/prefetched.json --out [keyword]/.cache/enriched.json
```
